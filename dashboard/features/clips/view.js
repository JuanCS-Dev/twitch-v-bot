...
export function renderVisionStatus(payload, els) {
  ...
  if (els.visionFramesCount)
    els.visionFramesCount.textContent = payload.frame_count || 0;

  if (els.visionLastIngest) {
    if (payload.last_ingest_at && payload.last_ingest_at > 0) {
      els.visionLastIngest.textContent = new Date(
        payload.last_ingest_at * 1000,
      ).toLocaleTimeString();
    } else {
  ...
  if (els.visionLastAnalysis) {
    els.visionLastAnalysis.textContent =
      payload.last_analysis || "No analysis available.";
  }
