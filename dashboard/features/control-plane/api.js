...
export async function getAgentNotes(channelId, timeoutMs = REQUEST_TIMEOUT_MS) {
  const safeChannelId = encodeURIComponent(
    String(channelId || "")
      .trim()
      .toLowerCase(),
  ...
  return await fetchWithTimeout(
    `${AGENT_NOTES_ENDPOINT}?channel=${safeChannelId}`,
    { method: "GET", headers: authHeaders() },
    timeoutMs,
  );
}

export async function updateAgentNotes(
  payload,
  timeoutMs = REQUEST_TIMEOUT_MS,
) {
  return await fetchWithTimeout(
    AGENT_NOTES_ENDPOINT,
    {
      method: "PUT",
      headers: authHeaders(),
      body: JSON.stringify(payload || {}),
