...
function renderSummaryAutonomyState(runtime, budget, els) {
  const stateLabel = runtime.suspended
    ? "SUSPENDED"
    : runtime.enabled
      ? "ON"
      : "OFF";
  const loopLabel = runtime.loop_running ? "running" : "idle";
  setText(els?.summaryAutonomyState, `${stateLabel} - ${loopLabel}`);
  setText(
    els?.summaryAutonomyBudget,
    `Budget 60m: ${formatNumber(budget.messages_60m || 0)}/${formatNumber(budget.limit_60m || 0)}`,
  );
}
...
export function renderAutonomyRuntime(runtimePayload, els) {
  ...
  setText(
    els?.budgetBlockedTotal,
    formatNumber(runtime.autonomy_budget_blocked_total),
  );
  setText(
    els?.dispatchFailuresTotal,
    formatNumber(runtime.autonomy_dispatch_failures_total),
  );
  setText(
    els?.autoChatSentTotal,
    formatNumber(runtime.autonomy_auto_chat_sent_total),
  );
  ...
  renderSummaryAutonomyState(runtime, budget, els);
