...
def compute_chat_metrics(
    ...
    avg_len_10m = round(sum(int(e.get("length", 0)) for e in events_10m) / count_10m, 1) if count_10m else 0.0
    avg_len_60m = round(sum(int(e.get("length", 0)) for e in events_60m) / count_60m, 1) if count_60m else 0.0
...
def compute_quality_metrics(
    ...
    success_60m = sum(1 for e in events_60m if str(e.get("outcome", "")).strip().lower() == "retry_success")
    fallback_60m = sum(1 for e in events_60m if str(e.get("outcome", "")).strip().lower() == "fallback")
...
def compute_autonomy_metrics(
    ...
    ignored_60m = sum(1 for e in events_60m if str(e.get("outcome", "")).strip().lower() == "ignored")
...
def compute_leaderboards(
    ...
    top_chatters_60m = [{"author": a, "messages": c} for a, c in chatters_60m.most_common(LEADERBOARD_LIMIT)]
    top_chatters_total = [{"author": a, "messages": c} for a, c in Counter(chatter_totals).most_common(LEADERBOARD_LIMIT)]
    top_triggers_60m = [{"author": a, "triggers": c} for a, c in triggers_60m.most_common(LEADERBOARD_LIMIT)]
    top_triggers_total = [{"author": a, "triggers": c} for a, c in Counter(trigger_totals).most_common(LEADERBOARD_LIMIT)]
