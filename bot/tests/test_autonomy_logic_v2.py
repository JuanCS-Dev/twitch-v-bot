...
from bot.autonomy_logic import generate_goal_text, process_autonomy_goal
...
class TestAutonomyLogicV2(unittest.IsolatedAsyncioTestCase):
    ...
    @patch(
        "bot.autonomy_logic.control_plane.can_send_auto_chat", return_value=(False, "cooldown", {})
    )
    ...
    @patch(
        "bot.autonomy_logic.control_plane.get_config", return_value={"clip_pipeline_enabled": False}
    )
    ...
    @patch(
        "bot.autonomy_logic.control_plane.get_config", return_value={"clip_pipeline_enabled": True}
    )
