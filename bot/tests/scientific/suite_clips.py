...
class TestTwitchClipsAPI(unittest.TestCase):
    ...
    def test_create_clip_live_success(self):
        mock_ctx = self._mock_urlopen(202, {"data": [{"id": "Clip123", "edit_url": "http://edit"}]})
    ...
    def test_get_clip_found(self):
        mock_ctx = self._mock_urlopen(200, {"data": [{"id": "Clip123", "url": "http://clip"}]})
    ...
    def test_get_clip_processing(self):
        mock_ctx = self._mock_urlopen(200, {"data": []})
...
class TestClipJobsRuntime(unittest.TestCase):
    ...
    def test_sync_from_queue(self):
        ...
        with patch(
            "bot.control_plane.control_plane.list_actions", return_value={"items": [mock_item]}
        ):
...
class TestAutonomyLogic(unittest.TestCase):
    def test_process_autonomy_goal_clip_candidate(self):
        ...
        with (
            patch(
                "bot.autonomy_logic.generate_goal_text",
                return_value="Um momento incrivel aconteceu",
            ),
            patch(
                "bot.control_plane.control_plane.get_config",
                return_value={"clip_pipeline_enabled": True},
            ),
    ...
    def test_process_autonomy_goal_clip_disabled(self):
        ...
        with (
            patch("bot.autonomy_logic.generate_goal_text", return_value="Clip this"),
            patch(
                "bot.control_plane.control_plane.get_config",
                return_value={"clip_pipeline_enabled": False},
            ),
        ):
            result = asyncio.run(process_autonomy_goal(goal, None))
