...
from bot import irc_connection
...
class TestIrcConnection(unittest.IsolatedAsyncioTestCase):
    ...
    @patch.object(
        irc_connection.IrcConnectionMixin, "_await_login_confirmation", new_callable=AsyncMock
    )
    ...
    async def test_run_forever_reconnect_on_error(self):
        bot = MockBot()
        with patch.object(
            bot, "_connect", side_effect=[Exception("First fail"), asyncio.CancelledError()]
        ):
