...
**Status:** Fases antigas (1-13) + Fases 14-15 concluídas ✅ | Backlog ativo: Fases 16-19 + otimização `pgvector` (futuro)
...
1. **Fase 16 - Coaching em Tempo Real + Viewer Churn Risk:** alertas táticos no HUD/dashboard com antirruído.
2. **Fase 17 - Revenue Attribution Trace:** correlação temporal entre ação do agente e conversão (follow/sub/cheer).
3. **Fase 18 - Outbound Webhook API:** destinos, assinatura/HMAC, retry e observabilidade de entrega.
4. **Fase 19 - Autonomous Clip Suggestion Intelligence:** detecção de momento clipável acoplada ao pipeline já existente.
5. **Evolução futura:** otimização ANN com `pgvector` para memória semântica (escala/performance).
